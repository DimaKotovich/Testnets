...
class Opside:
    def __init__(self):
        ...
        self.faucet_url = 'https://faucet-testnet.opside.network/'
        self.switch_network_url = 'https://chainlist.org/chain/23118'
        self.staking_url = 'https://staking-testnet.opside.network/'
        self.bridge_url = 'https://zkbridge-testnet.opside.network/'
        self.liquidity_url = 'https://testnet.normalswap.finance/'
        self.discord_invite = 'https://discord.com/invite/bsbMDwUuYQ'
        self.driver = None
        self.wallet = None
    ...
    def sign_metamask(self):
        try:
            WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '/html/body/div[1]/div/div[2]/div/div[3]/button[2]'))).click()
        except TimeoutException:
            print('sign_metamask: Error button to found')

    def faucet(self):
        self.check_current_page(self.faucet_url)
        try:
            connect_button = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '//*[@id="__nuxt"]/div/div[1]/div/div/div/div[2]/button')))
            if connect_button.text == 'Connect Wallet':
                ...
                try:
                    ...
                    print('faucet: Error click second button to connect wallet')

            WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '/html/body/div[2]/div/div[2]/div/div/div[2]/div/div[2]/button'))).click()
            time.sleep(1)
            self.wallet.switch_page_to_action('MetaMask Notification', self.sign_metamask)
        except TimeoutException:
            print('faucet: Error all actions')

    def confirm_staking(self):
        try:
            WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '/html/body/div[1]/div/div[2]/div/div[3]/div[3]/footer/button[2]'))).click()
        except TimeoutException:
            pass

    def staking(self):
        self.check_current_page(self.staking_url)
        try:
            connect_button = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '//*[@id="__nuxt"]/div/div[1]/div/div/div/div/button')))
            if connect_button.text == 'Connect Wallet':
                connect_button.click()
                try:
                    WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                    ...
                    print('staking: Error click second button to connect wallet')
        except TimeoutException:
            print('staking: Error while checking wallet connection')

        try:
            WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '/html/body/div[2]/div/div[3]/div/div/div/div/div[2]/div[1]/div[2]/div[1]/div/div/div['
                           '1]/div[3]/div/div[1]/div/table/tbody/tr[1]/td[5]/div/div/button'))).click()
            time.sleep(3)
            input_amount = WebDriverWait(self.driver, 10).until(EC.presence_of_element_located(
                (By.XPATH, '/html/body/div[4]/div/div/div/div/div[1]/div[2]/div[1]/input')))
            input_amount.send_keys('1')
            WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '/html/body/div[4]/div/div/div/div/button'))).click()
            time.sleep(1)
            self.wallet.switch_page_to_action('MetaMask Notification', self.confirm_staking)
            status = WebDriverWait(self.driver, 10).until(EC.presence_of_element_located(
                (By.XPATH, '/html/body/div[4]/div/div/div/div/h1')))
            if status.text != 'Delegate Completed':
                pass
        except TimeoutException:
            print('staking: Error when staking')

    def bridge(self):
        self.check_current_page(self.bridge_url)
        try:
            connect_button = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '//*[@id="__nuxt"]/div/div[1]/div/div/div/div[2]/button')))
            ...
            print('bridge: Error while checking wallet connection')

        try:
            WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '/html/body/div[5]/div/div[2]/div/div/div/div/div[2]/div[1]/div[3]/div/div/div'))).click()
            WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '/html/body/div[2]/div/div/div/button[2]'))).click()
            balance = WebDriverWait(self.driver, 10).until(EC.presence_of_element_located(
                (By.XPATH, '/html/body/div[5]/div/div[2]/div/div/div/div/div[2]/div[2]/div[2]/div[2]/span'))).text
            balance = float(balance[0:len(balance)-3]) - 0.00075
            input_amount = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '/html/body/div[5]/div/div[2]/div/div/div/div/div[2]/div[2]/div[1]/div/div[1]/input')))
            input_amount.send_keys(balance)
            WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '/html/body/div[5]/div/div[2]/div/div/div/div/div[2]/button'))).click()
            time.sleep(3)
            self.wallet.switch_page_to_action('MetaMask Notification', self.confirm_staking)
            # status = WebDriverWait(self.driver, 10).until(EC.presence_of_element_located(
            #     (By.XPATH, '/html/body/div[4]/div/div/div/div/h1')))
    ...
    def liquidity(self):
        self.check_current_page(self.liquidity_url)
        try:
            connect_button = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '//*[@id="__nuxt"]/div/div[1]/div/div/div[2]/div[2]/button')))
            print(connect_button.text)
            if connect_button.text == 'Connect Wallet':
                connect_button.click()
                try:
    ...
    def discord(self):
        self.check_current_page(self.discord_invite)
        try:
            WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '/html/body/div[1]/div[2]/div[1]/div[1]/div/div/div/div/section/div[2]/button'))).click()
            WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '/html/body/div[1]/div[2]/div[1]/div[1]/div/div/div/div/section/div[2]/button'))).click()
        except TimeoutException:
            pass
    ...
    def start_WebDriver(self, id):
        ...
        self.close_all_tabs()
        self.wallet = MetaMask(self.driver)
        self.wallet.open_wallet()
        if not self.wallet.session_login:
            self.wallet.login_password()
        self.wallet.close_modal(3, '//*[@id="popover-content"]/div[2]/div/section/div[1]/div/button')
        self.wallet.close_modal(3, '//*[@id="popover-content"]/div/div/section/div[3]/button')
        self.wallet.show_test_networks()
        self.wallet.check_network()
        if self.wallet.current_network != self.wallet.opside_testnet:
            result = self.wallet.switch_network(self.wallet.opside_testnet)
            if result == 'Not Found network':
                self.wallet.add_network_advanced(
                    name='Opside Testnet',
                    url='https://testrpc.opside.network',
                    id=23118,
                    symbol='IDE',
                    explorer='https://opside.info'
                )
            self.wallet.switch_network(self.wallet.opside_testnet)
    ...
    def accounts_multiprocess(self):
        ...
        try:
            for index in range(1, len(self.book['proxy_name']) + 1):
                ...
                self.session_wallet = False
                if self.data['result'][index] == 'Yes':
                    print(f'Account: {index} performed all actions')
                    continue
                if self.data['errors'][index] != 'No':
                    self.data['errors'][index] = 'No'
