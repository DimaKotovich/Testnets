...
class BaseTestnet:
    def __init__(self):
        ...
        self.main_url = 'https://www.karate.com/airdrop'
        self.driver = None
    ...
    def sign_email(self, index):
        self.check_current_page(self.main_url)
        try:
            input = WebDriverWait(self.driver, 10).until(EC.presence_of_element_located(
                (By.XPATH,
                 '//*[@id="__next"]/div/div[3]/section/div/div[2]/div/form/div/input')))
            input.send_keys(self.book['gmail'][index])
            WebDriverWait(self.driver, 10).until(EC.presence_of_element_located(
                (By.XPATH,
                 '//*[@id="__next"]/div/div[3]/section/div/div[2]/div/form/h5/button'))).click()
            self.data['sign_email'][index] = 'Yes'
            self.data.to_csv('./data/save.csv', sep=';')
        except TimeoutException:
            print('sign_email: Elements not Found')
    ...
    def accounts_multiprocess(self):
        ...
        try:
            for index in range(1, len(self.book['proxy_name']) + 1):
                ...
                if self.data['sign_email'][index] == 'Yes':
